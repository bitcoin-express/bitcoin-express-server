...
const DECIMAL_POINT_PRECISSION = config.get('system.decimal_point_precision');
 ...
 */
exports.coinSelection = function(target, coins, args) {
...
//EXIT POINT ********
  ...
  let verifiedValue = round(faceValue - verificationFee.totalFee, DECIMAL_POINT_PRECISSION);
...
//Find the smallest single coin that can completely satisfy the target value after the fee is deducted
  ...
  if (defaultCandidateIndex >= 0) {
    ...
    if (wkCoins.length > 0) {
      ...
      verifiedValue = round(faceValue - verificationFee.totalFee, DECIMAL_POINT_PRECISSION);
    } else {
      faceValue = 0;
      verifiedValue = 0;
    }
...
//Activity: 'consolidate small coins'
  let valueAttractingMinFee = round(((issuer.feeMin - issuer.feeFixed) / issuer.feeVariable), DECIMAL_POINT_PRECISSION);
  ...
  joinCandidateSum = round(joinCandidateSum, DECIMAL_POINT_PRECISSION);
...
//Activity: 'pre-select coins'
  ...
  while(wkCoins.length > COMBINATION_SEARCH) {
      ...
      faceValue = round(preSelectedSum + wkCoins[largestCoinIndex].value, DECIMAL_POINT_PRECISSION);
  ...
  if(preSelectedSum > 0) { //adjust the target and possibly remove larger coins
      ...
      faceValue = round(preSelectedSum + joinCandidateSum, DECIMAL_POINT_PRECISSION);
      if(args.singleCoin) {
...
//Activity: 'preselected + joinCandiates is sufficient' TEST CASE#14
          ...
          let tv = args.singleCoin ? target : round(target - preSelectedSum, DECIMAL_POINT_PRECISSION);
...
//Activity: 'adjust target for pre-selected coins'
      target = round(target - preSelectedSum, DECIMAL_POINT_PRECISSION);
...
//EXIT POINT ********
          ...
          let tv = round(realTarget - preSelectedSum, DECIMAL_POINT_PRECISSION);
          if(tv > coinMinValue && tv < (joinSum - verificationFee.totalFee)) {
              faceValue = round(preSelectedSum + joinSum, DECIMAL_POINT_PRECISSION);
              verifiedValue = round(faceValue - verificationFee.totalFee, DECIMAL_POINT_PRECISSION);
...
//EXIT POINT ********
  ...
  let pwrSearch = function(element) {
      let val = round(target - element.s, DECIMAL_POINT_PRECISSION);
      if(val <= 0) {
          return false;
      }
...
//Activity: 'pre-select join candidates'
  if(joinCandidateSum > 0) {
      ...
      target = round(target - joinCandidateSum, DECIMAL_POINT_PRECISSION); //reduce the target by the join candidate sum
...
//This time we are looking for combinations that don't include the candidateToSplit
          ...
          if(matchList != null) { //we may have a hit
              for(let j=0; j<matchList.length; j++) {
                  ...
                  let tv = round(realTarget - sum, DECIMAL_POINT_PRECISSION);
                  if(tv >= coinMinValue) {//Issuer will reject any target value smaller than minCoinValue
                      faceValue = round(sum + candidateToSplit.value, DECIMAL_POINT_PRECISSION);
                      verifiedValue = round(faceValue - candidateToSplit.fee, DECIMAL_POINT_PRECISSION);
                      if(verifiedValue >= realTarget) {
...
//Activity: 'revert to reduced target default candidate - multi coin' TEST CASE#8
          ...
          let tv = round(realTarget - sum, DECIMAL_POINT_PRECISSION);
          verifiedValue = round(reducedTargetDefaultCandidate.verifiedValue + sum, DECIMAL_POINT_PRECISSION);
          if(tv >= coinMinValue && tv <= verifiedValue) {
              faceValue = round(reducedTargetDefaultCandidate.value + sum, DECIMAL_POINT_PRECISSION);
...
//Activity: 'revert to all coins - multi coin' TEST CASE#13
      ...
      let coinToSplitIndex = allCoins.findIndex(function(element) {
          ...
          tv = round(realTarget - remainder, DECIMAL_POINT_PRECISSION);
      ...
      verifiedValue = round((coinToSplit === null ? 0 : coinToSplit.verifiedValue) + _arraySum(selection, "value", 0, allCoins.length), DECIMAL_POINT_PRECISSION);
...
function _archiveCoinSelection(description, target, startTime, cycles, numberOfCoins, coinSelectionResponse) {
  ...
  str += "] (sum="+round(sum, DECIMAL_POINT_PRECISSION)+")";
  arch.selection = str;
...
function _arraySum(array, item, begin, n) {
  ...
  return round(sum, DECIMAL_POINT_PRECISSION);
}
...
function Coin(base64, includeFee, args, passphrase) {
  try {
    ...
    obj.base64 = base64;
    obj.value = round(parseFloat(obj.v), DECIMAL_POINT_PRECISSION);

    if (includeFee) {
      let fees = _getVerificationFee(obj, args);
      obj.fee = round(fees.totalFee, DECIMAL_POINT_PRECISSION);
      obj.verifiedValue = round(obj.value - obj.fee, DECIMAL_POINT_PRECISSION);
    }
    return obj;
  } catch(err) {
    return null;
...
function _getVerificationFee(aCoin, args) {
  ...
  if (!("value" in aCoin)) {
    aCoin.value = round(Number.parseFloat(aCoin.v), DECIMAL_POINT_PRECISSION);
  }
...
function _calcVerificationFee(value, inCoinCount, args) {
  ...
  let expiryFee = round(expiryHours * Number.parseFloat(args.issuerService.feeExpiry), DECIMAL_POINT_PRECISSION);

  let expiryEmailFee = 0;
  if ("expiryEmail" in args && args.expiryEmail.length > 0) {
    expiryEmailFee= round(Number.parseFloat(args.issuerService.feeExpiryEmail), DECIMAL_POINT_PRECISSION);
  }

  let variableFee  = round(value * Number.parseFloat(args.issuerService.feeVariable), DECIMAL_POINT_PRECISSION);
  ...
  return {
    variableFee: round(variableFee, DECIMAL_POINT_PRECISSION),
    totalFee: round(totalFee, DECIMAL_POINT_PRECISSION)
  };
}
...
function _setVerifiedValue(coinObj, args) {
  ...
  coinObj.value = round(value, DECIMAL_POINT_PRECISSION);
  
  let fees = _getVerificationFee(coinObj,args);
  coinObj.fee = fees.totalFee;
  coinObj.verifiedValue = round(value - fees.totalFee, DECIMAL_POINT_PRECISSION);
  coinObj.variableFee = fees.variableFee;
  coinObj.variableValue = round(value - fees.variableFee, DECIMAL_POINT_PRECISSION);
}
...
function _arrayTotalValue(arr, args, verifyAllCoins) {
  ...
  return round(sum - fees.totalFee, DECIMAL_POINT_PRECISSION);
}
...
function _sumCoinValue(src, arr) {
