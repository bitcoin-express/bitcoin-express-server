/*
    ...
    Special keys:
        * _system_required_keys:
            app.js before server initialisation will check if all essential settings, mentioned in this key, are set and
            throw an error if configuration is not full,
        * _account_readonly_settings:
            Account settings that are accessible/visible by users but can't be overwritten. These keys can't be modified
            via setConfig method although they will be returned in getSettings set,
...
module.exports = {
    ...
    server: {
        ...
        api: {
            ...
            // Time (in seconds) to wait before retrying after potentially-recoverable API error
            soft_error_retry_delay: 5,

            // Maximum time (in seconds) given for the API request to conclude. API may use it to terminate the request after
            // this time if not able to finish the request.
            time_budget: 60,

            // Buffer delaying post-time_budget operations on the API level. API may perform clean up operations after
            // time_budget is depleted and this buffer may be used to delay this process allowing current operation to finish.
            time_budget_trigger_buffer: 3,

            // Maximum number of retries that the API should call Issuer's endpoints
            issuer_call_retries: 3,

            // Email to be used as a recovery measure for Issuer related API calls. If something breaks during
            // communication with an Issuer and there are Coins involved in the process this is where an Issuer will
            // send remaining/recovered Coins
            issuer_recovery_email: undefined,

            // Password to encrypt Coins with on the Issuer side before sending them to recovery email
            issuer_recovery_password: "",
        },
    ...
    system: {
        ...
        issuer_verify_call_expiry: 3600,
    },
};
